...
/**
 * 赋值函数
 * @param paramObj 自己定义的需要提交给后端参数值
 * @param getObj 接口获取的参数值或者表单里自定义的值
 * 参数值不一样的，组件中单独添加
 */


export function cloneParmas(getObj, paramObj) {
  const getArr = Object.keys(getObj)
  const obj = {}
  for (const key in paramObj) {
    const element = paramObj[key];
    if (getArr.includes(key)) {
      obj[key] = element
