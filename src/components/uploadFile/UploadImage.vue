...
<template>
  <div style="display: flex" id="UploadImage">
    <div v-if="isUpdate && isEdit">
      <el-upload drag action="string" ref="uploadImgRef" list-type="picture-card" :fileList="fileList"
        :http-request="uploadImage" :multiple="true" :limit="limit" :before-upload="beforeAvatarUpload"
        :on-success="handleAvatarSuccess" :on-exceed="handleExceed" :on-error="imgUploadError">
      ...
      <span class="el_dialog_image">
        <el-dialog v-model="dialogVisible" :close-on-click-modal="false" destroy-on-close>
    ...
    <div v-else>
      <span v-for="(item, index) of fileList" :key="index" class="imgBox">
        <i v-if="!isUpdate && type !== 3 && type !== 6 && type !== 8 && type !== 4"
          class="el-icon-circle-close filebadge" type="primary" @click="removeImg"></i>
