...
<template>
    <div v-dialogdrag>
        <el-dialog :title="title ? title : '导入'" width="600px" v-model="addVisible" :close-on-click-modal="false" destroy-on-close>
            <div class="allAdd-box">
                <div>
                    ...
                    <div style="margin-top:23px;">
                        <el-radio-group v-model="funMsg.istype" class="ml-4">
                            <el-radio label="2" size="large">补充</el-radio>
        ...
        <el-dialog title="提示" width="30%" v-model="promptVisible" :close-on-click-modal="false" destroy-on-close>
            <div class="info-box">
                <p class="info-top">{{  infoValue.resMessage  }}</p>
                <p>详细描述</p>
                <ul style="margin-top: 10px">
                    <li style="margin-bottom: 5px" v-for="(item, index) in infoValue.errorMessages" :key="index">{{  item 
                        }}</li>
                </ul>
            </div>
        </el-dialog>
...
export default {
    ...
    setup(prop, ctx) {
        ...
        const downloadTemp = () => {
            ...
            downloadTemplate(data.funMsg.temp, data.funMsg.title, data.funMsg.type ? { type: data.funMsg.type } : data.funMsg.orderCode ? data.funMsg.orderCode :data.funMsg.excelType? {excelType:data.funMsg.excelType} : []);
        };
        ...
        const uploadChange = () => {
            ...
            if (data.funMsg.excelType) {
                excelData.append("excelType",data.funMsg.excelType)
            }
            excelData.append('type', data.funMsg.istype)
            if (data.funMsg.istype == "1") {
                vue.$confirm(
                    `正在执行负责人导入覆盖，覆盖后无法恢复请谨慎操作`,
                    "提示",
                    {
