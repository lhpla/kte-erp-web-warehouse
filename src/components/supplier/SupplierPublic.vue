        ...
        <div class="list" :style="{ height: dataLength * itemHeight + 'px' }">
          <ul v-loading="labelLoading" :style="{ 'margin-top': `${scrollTop}px` }">
            <li
              ...
              :key="item.id"
              :class="supplierName == item.supplierName ? 'activeClass' : ''"
...
<script>
import { reactive, toRefs, computed, onMounted, getCurrentInstance, onActivated, watch, nextTick } from "vue";
...
export default {
  ...
  setup(props, ctx) {
      ...
      if (e && e != "") {
        ...
        if (document.querySelector(".virtual .list") != null) {
          nextTick(() => {
            ...
            document.querySelector(".virtual .list").style.height = data.dataLength * 40 + "px";
          });
        }
      } else {
        data.startIndex = 0;
    ...
    return { ...toRefs(data), scroll, visiablelist, itemHeight, Select, changeName, getName, changeSupplierName, getSupplier, reastValue, showClick };
  },
};
</script>
