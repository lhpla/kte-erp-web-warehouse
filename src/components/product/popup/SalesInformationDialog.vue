<!-- SalesInformationDialog 销售信息 -->
<template>
  <div id="SalesInformationDialog">
    <el-dialog title="销售信息" width="60%" v-model="SalesInfoDialogVisiable" :close-on-click-modal="false" @close="closed" destroy-on-close>
        ...
        <template v-if="proMarketInfoList.length > 0">
          <div v-for="(item, index) in proMarketInfoList" :key="index" class="form-border">
            <el-form
              ...
              v-show="!formLoading"
            >
              <el-button class="del-productAttributes" icon="el-icon-delete" @click="delAttribut(index, item.id)" type="text"
                >删除</el-button
              >
              ...
              <el-form-item label="销售站点:" prop="siteId">
                <el-select v-model="item.siteId" size="mini" filterable clearable placeholder="全站点">
              ...
              <el-form-item label="销售负责人:" prop="salesManagerId">
                <el-select v-model="item.salesManagerId" size="mini" filterable clearable placeholder="请选择">
              ...
              <div>
                <el-form-item style="position: relative" label="海外参考链接:" prop="overseasReferenceLink">
                  <el-input clearable v-model.trim="item.overseasReferenceLink" placeholder="http://...."></el-input>
                  <i
                ...
                <el-form-item style="position: relative" label="海外销售链接:" prop="overseasSalesLink">
                  <el-input clearable v-model.trim="item.overseasSalesLink" placeholder="http://...."></el-input>
                  <i
                    style="position: absolute; right: -15px; top: 9px"
              ...
              <span class="formDis">
                <el-form-item label="建议售价:" prop="offerPrice">
                  <el-input clearable v-model.trim="item.offerPrice" class="size-width" placeholder="9.85"></el-input>
                </el-form-item>
                <el-form-item label="" prop="priceUnit" style="display: flex">
                  <el-select v-model="item.priceUnit" size="mini" filterable clearable placeholder="USD(美元)">
                    <el-option
...
export default {
  name: "SalesInformationDialog",
  setup(prop, ctx) {
    const data = reactive({
      SalesInfoDialogVisiable: false,
      rules: {
    ...
    // 修改
    const getApiMsg = (msg) => {
      data.SalesInfoDialogVisiable = true;
      getSaleMsg();
        ...
        if (res.code == 200) {
          if (res.data.length > 0) {
            ...
            for (let i = 0; i < data.proMarketInfoList.length; i++) {
              ...
              data.proMarketInfoList[i].offerPrice =
                data.proMarketInfoList[i].offerPrice > 0 ? Number(data.proMarketInfoList[i].offerPrice).toFixed(2) : 0;
              data.proMarketInfoList[i].salesManagerId =
                data.proMarketInfoList[i].salesManagerId > 0 ? data.proMarketInfoList[i].salesManagerId : "1474632793794334722";
              ...
              data.proMarketInfoList[i].affiliateSalesManager =
                data.proMarketInfoList[i].affiliateSalesManager.length > 0
                  ? data.proMarketInfoList[i].affiliateSalesManager.split(",")
                  : [];
              // data.proMarketInfoList[i].siteList = getStateList(data.proMarketInfoList[i].platformId);
              if (data.proMarketInfoList[i].platformId) {
                api.system
                  .getSiteByPlat({
                    platformId: data.proMarketInfoList[i].platformId,
                    isEnglish: false,
    ...
    const closed = () => {
      data.SalesInfoDialogVisiable = false;
      data.proMarketInfoList = [
        {
          platformId: "",
