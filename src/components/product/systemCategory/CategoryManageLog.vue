<!--CategoryManageLog   产品品类操作日志-->
<template>
  <div id="CategoryManageLog">
    <el-dialog title="操作日志" width="85%" v-model="logVisible" :close-on-click-modal="false" @close="closed" destroy-on-close>
      <el-table :header-cell-style="rowClass" :data="logTableData" tooltip-effect="dark" element-loading-text="加载中" v-loading="loading" border>
        ...
        <el-table-column prop="operator" align="center" label="操作人名称" width="200"></el-table-column>
        <el-table-column prop="updateTime" align="center" label=" 创建时间" width="200"></el-table-column>
      </el-table>
      <el-pagination
...
export default {
  ...
  setup(prop, ctx) {
    ...
    const handleLog = id => {
      ...
      api.product.categoryGetLogById(logData).then(res => {
        if (res.code == 200) {
